...
FAST_file=fopen("./data_2017-06-06_16-36-36","r");
WriteHeader("format_test");

fil_file=fopen("./format_test.fil","a+");
//fwrite(&a,sizeof(a),1,FAST_file);
for(int i=0;i<10;i++){
	//fread(&counter,sizeof(long int),1,FAST_file);
	fread(raw_data,sizeof(short int),2048,FAST_file);
	for(int j=0;j<512;j++){
